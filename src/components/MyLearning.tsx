...
const backendBaseUrl =
  ...
  "https://offficial-website-backend-production.genomac.io";

const resolveMediaUrl = (value?: string, fallback?: string) => {
  ...
  return `${backendBaseUrl}${candidate.startsWith("/") ? "" : "/"}${candidate}`;
};

const MyLearning = () => {
  const { data: userData } = useUserDetail();

  const userTag = userData?.tags === "None" ? "" : userData?.tags || "";
  const { data } = useFetchVideoForUser(userTag);
  const videos = data || [];
  ...
  if (userData?.tags === "None") {
    return (
      <div className="h-[100vh] flex justify-center items-center text-[1.2rem]">
        No videos
      </div>
    );
  }
          ...
          <>
            ...
            <div className="p-6 bg-white min-h-[80vh] rounded-lg m-4">
              ...
              <div className="flex flex-wrap gap-5">
                ...
                >
                  {selectedVideo.isAvailable && <>G-iHUB</>}
                </span>
              </div>
            </div>
          </>
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        <span className="text-sm text-gray-500 mr-3">
                          {groupedVideos[sectionNumber].length} videos
                        </span>
                        <svg
                          className={`w-5 h-5 text-gray-500 transition-transform duration-200 ${
                            openSections[sectionNumber]
